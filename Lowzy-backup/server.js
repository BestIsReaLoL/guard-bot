...
mongoose.connect("Mongo yaz buraya", { useNewUrlParser: true, useUnifiedTopology: true })
...
client.on("message", async message => {
  ...
  if(message.content === ".role") {
   ...
   await roleData.findOne({rolid: role.id}, async(err,res) => {
      ...
      message.channel.send("Rol Yedeklendi \n```Rol: "+role.name+"\nÜye Sayısı: "+role.members.size+"```")
      } else if(res) {
          ...
          if(filter.length > 200) return client.channels.cache.get("Log id").send("@everyone Rol Bilgisi 🚫\n```Database: "+res.sayı+"\nRol: "+role.members.size+"\nİsim: "+role.name+"```")
          res.name = role.name;
  ...
  message.guild.roles.cache.map(async(role) => {
    ...
    await channelData.findOne({ roleid: role.id }, async (err, res) => {
      if (!res) {
        ...
        message.channel.send("Rol Kanal İzinleri Yedeklendi \n```Rol: "+role.name+"\nKanal sayısı: "+channels+"```", {split: true})
      } else {
        res.roleid = role.id;
...
client.on("roleDelete", role => {
  client.channels.cache.get("Rol silme log id").send("Silinen Rol ID: `" + role.id + "`");
  client.channels.cache.get("Rol acma log id").send("Silinen Rol İsim: `" + role.name + "`");
});
...
client.on("message", async msg => {
  ...
  if (!msg.guild) return;
  let prefikslerim = [".", "e.", "e!"];
  let lowzy = false;
  for (const içindeki of prefikslerim) {
    if (msg.content.startsWith(içindeki)) lowzy = içindeki;
  }
  if (!lowzy) return;
  ...
  switch (command) {
    case "rolsilindi":
      if (msg.author.id !== "kendi id") return;
